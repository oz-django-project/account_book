...
class Analyzer:
    def __init__(self, user: User, about: str, analysis_type: str, start_date: date, end_date: date):
        self.user = user
    ...
    def _get_user_display_name(self):
        return getattr(self.user, 'username', None) or getattr(self.user, 'email', '사용자')

    def _korean_label(self):
        type_map = {'weekly': '주간', 'monthly-category': '월간 카테고리별'}
        about_map = {
            'expense': '소비', 'income': '수입',
            'withdraw': '소비', 'deposit': '수입',
            'WITHDRAW': '소비', 'DEPOSIT': '수입',
        }
    ...
    def _visualize_weekly(self, df_grouped):
        ...
        if self.start_date == self.end_date:
            ax.set_xlim(self.start_date - timedelta(days=1), self.end_date + timedelta(days=1))
        else:
    ...
    def _visualize_category(self, df_grouped):
        ...
        ax.set_ylabel('금액')
        ax.set_xlabel('카테고리')
        ax.set_title(f"{self._get_user_display_name()}의 월간 카테고리별 {self._korean_label().split()[-1]} 분석")
        ax.set_xticks(x)
